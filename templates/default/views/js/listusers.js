...
$(document).delegate('label.user-active','click',function(){
	...
	$.ajax({
		...
		async : true,
		dataType:"json",
		data : { ajax : 1, m : 'linkmobility', act : 'user_set_active', id : user_id, value : check ? 0 : 1, s_id : session_id },
	...
	$('.user-delete-link').click(function(){
		...
		if(confirm('Valóban törli?')){
			...
			$.ajax({
				...
				async : false,
				dataType:"json",
				data : { ajax : 1, m : 'linkmobility', act : 'user_delete', id : id, s_id : session_id },
				success:function(data){
					console.log(data);
				}
			});
