...
require_once('DbEngines/users.db.php');
...
class user {
	...
	private function cell_edit($row) {
		...
		/*if((int)$row['UserType'] < (int)$this->user_level($_SESSION["HDT_uid"])) {
			...
			?>
				<a id="user_edit_<?php echo $row['ID'];?>" class="user-edit-link" data-id="<?php echo $row['ID'];?>" href="http://<?php echo ROOT_URL?>/?m=<?php echo MODULE_NAME;?>&act=edituser&id=<?php echo $row['ID'];?>"><i class="material-icons">border_color</i></a>
			<?php 
			} elseif($_SESSION["HDT_theme"] == 'default') { ?>
				<a id="user_edit_<?php echo $row['ID'];?>" class="user-edit-link" data-id="<?php echo $row['ID'];?>" href="http://<?php echo ROOT_URL?>/?m=<?php echo MODULE_NAME;?>&act=edituser&id=<?php echo $row['ID'];?>"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a>
			<?php
	...
	public function login($username = NULL, $password = NULL, $servicelogin = NULL) {
		...
		if($user != false) {
			...
			//var_dump($this->parent->get_component('user_logged')); exit;
