...
class Email {
	...
	const EMAIL_FROM = 'info@linkmob.test';
	const EMAIL_FROMNAME = 'Link Mobility próbafeladat';
	...
	public function __construct($parent = NULL) {
		...
		var_dump($this->parent->module_name);
		
		$this->phpmailer = new PHPMailer;
		$this->phpmailer->CharSet = 'UTF-8';
		if($_SERVER['HTTP_HOST'] === '127.0.0.1' || $_SERVER['HTTP_HOST'] === '195.228.35.4') {
	...
	public function change_status_email($mandate_id = null) {
		...
		$track_url = 'http://'.ROOT_URL.'?m=' . $this->parent->module_name . '&act=mandate_tracking&uname='.$tracking['Uname'].'&passw='.$tracking['Passw'];
